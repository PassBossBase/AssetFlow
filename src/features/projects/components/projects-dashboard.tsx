...
function createProjectDragPreview(
  project: ProjectWithAssetCount,
  description: string,
  ...
  preview.className =
    "pointer-events-none fixed left-0 top-0 z-[100] min-h-28 overflow-hidden rounded-2xl border border-white/[0.2] bg-[#102037] p-4 opacity-100 shadow-[0_24px_56px_rgba(0,0,0,0.38),inset_0_1px_0_rgb(255_255_255_/_0.09)] will-change-transform";
  ...
  assetCount.className =
    "shrink-0 rounded-md border border-primary/25 bg-primary/[0.1] px-2 py-1 text-[11px] font-medium text-primary";
  ...
  previewDescription.className =
    "mt-3 line-clamp-2 text-sm leading-5 text-muted-foreground";
...
function ProjectAssetPreview({
  ...
  if (assetCount === 0) {
    return (
      <button
        type="button"
        className="flex h-28 w-full items-center justify-center rounded-xl border border-dashed border-white/[0.12] bg-white/[0.018] text-xs font-medium text-muted-foreground transition-colors hover:border-primary/40 hover:bg-primary/[0.045] hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        onClick={onUpload}
      >
  ...
  if (assets.length === 2) {
    return (
      <div
        className="grid h-28 grid-cols-2 gap-1.5 overflow-hidden rounded-xl"
        aria-label={t("recentAssets")}
      >
        {assets.map((asset) => (
          <ProjectAssetThumbnail key={asset._id} asset={asset} />
        ))}
      </div>
    );
  }
  ...
  if (assets.length === 3) {
    return (
      <div
        className="grid h-28 grid-cols-[2fr_1fr] gap-1.5 overflow-hidden rounded-xl"
        aria-label={t("recentAssets")}
      >
        <ProjectAssetThumbnail asset={assets[0]} />
  ...
  return (
    <div
      className="grid h-28 grid-cols-4 gap-1.5 overflow-hidden rounded-xl"
      aria-label={t("recentAssets")}
    >
      {assets.map((asset) => (
        <ProjectAssetThumbnail key={asset._id} asset={asset} />
    ...
    <div className="min-w-0 overflow-hidden rounded-lg border border-white/[0.07] bg-[#0a1320]">
      {previewUrl !== null && !hasError ? (
        ...
        <div className="flex size-full flex-col items-center justify-center gap-1 text-muted-foreground">
          ...
          <span className="max-w-full truncate px-2 text-[10px] font-medium uppercase">
            {asset.extension}
          </span>
        </div>
      )}
...
function ProjectCard({
  ...
  const isDropTarget = dropTargetProjectId === project._id && draggingProjectId !== project._id;
  ...
  function handlePointerDown(event: ReactPointerEvent<HTMLButtonElement>) {
    ...
    const preview = createProjectDragPreview(
      project,
    ...
    const updatePreview = (pointerEvent: PointerEvent) => {
      ...
      dragFrameRef.current = requestAnimationFrame(() => {
        ...
        const targetProjectId = projectIdAtPoint(
          pendingPosition.clientX,
          pendingPosition.clientY,
        );
        const nextTargetProjectId =
          targetProjectId === project._id ? null : targetProjectId;
  ...
  return (
    <div ref={cardRef} className="relative" data-project-id={project._id}>
      {isDropTarget ? (
        <>
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 z-10 rounded-2xl border-2 border-dashed border-white shadow-[0_0_0_3px_rgb(255_255_255_/_0.12),0_0_28px_rgb(255_255_255_/_0.16)]"
          />
        </>
      ) : null}
      <Card
      ...
      >
        <CardHeader className="p-5 pb-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              <span
                className="inline-flex items-center gap-1 rounded-md border border-primary/20 bg-primary/[0.07] px-2 py-1 text-xs font-medium text-primary"
                title={`${project.assetCount} ${t("assetCount")}`}
              >
          ...
          <div className="min-h-10">
            <CardDescription
              className={
                hasUploadingTasks || hasFailedTasks
                  ? "line-clamp-1"
                  : "line-clamp-2"
              }
            >
            ...
            {hasUploadingTasks ? (
              ...
              >
                <Upload
                  ...
                  aria-hidden="true"
                />
                <span className="min-w-0 shrink-0 text-[11px] font-medium text-primary">
                  {uploadProgressLabel}
                </span>
                <span
                  className="h-1 min-w-8 flex-1 overflow-hidden rounded-full bg-white/[0.09]"
                  aria-hidden="true"
                >
                  <span
                    className="block h-full rounded-full bg-primary transition-[width] duration-200"
                    style={{ width: `${project.uploadSummary.progress}%` }}
                  />
                </span>
              </div>
            ) : null}
            {completedUploadNotice !== undefined ? (
              <p
                className={`mt-2 flex items-center gap-1.5 text-[11px] font-medium text-emerald-400 transition-[opacity,transform] duration-300 motion-reduce:transition-none ${completedUploadNotice.isDismissing ? "-translate-y-1 opacity-0" : "translate-y-0 opacity-100"}`}
              >
                <Check className="size-3 shrink-0" aria-hidden="true" />
                {t("uploadCompleteCard").replace(
                  "{count}",
                  String(completedUploadNotice.count),
                )}
              </p>
            ) : null}
          </div>
        ...
        <CardContent className="space-y-3 px-5 pb-4">
          <ProjectAssetPreview
            assets={project.recentAssets}
            assetCount={project.assetCount}
            onUpload={() => onOpenUpload(project._id)}
          />
          {hasFailedTasks ? (
            <div className="space-y-2 rounded-xl border border-destructive/25 bg-destructive/[0.055] p-3">
              <p className="flex items-center gap-1.5 text-xs font-medium text-destructive">
                <AlertCircle className="size-3.5 shrink-0" aria-hidden="true" />
                {t("uploadsFailed").replace(
                  "{count}",
                  String(project.uploadSummary.failedCount),
                )}
              </p>
              {project.uploadSummary.failedTasks.map((task) => (
                <div
                  key={task._id}
                  className="flex items-center justify-between gap-3 rounded-lg bg-background/30 px-2.5 py-2"
                >
                  ...
                  <span className="flex shrink-0 items-center gap-1">
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      className="h-7 px-2 text-primary hover:bg-primary/[0.1] hover:text-primary"
                      onClick={() => onOpenUpload(project._id)}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => onRemoveUploadTask(task._id)}
                    >
...
function DeleteProjectButton({ project }: { project: Project }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.add({
        type: "success",
        title: t("projectDeletedNamed").replace("{name}", project.name),
      });
    } catch (deleteError) {
      toast.add({
        type: "error",
...
function NewProjectCard({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      className="group flex min-h-44 w-full flex-col items-start justify-between rounded-xl border border-dashed border-primary/40 bg-primary/[0.05] p-6 text-left transition-colors hover:border-primary hover:bg-primary/[0.1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.99]"
      onClick={onClick}
    >
      <span
        className="flex size-10 items-center justify-center rounded-lg border border-primary/40 bg-primary/10 text-2xl font-light text-primary"
        aria-hidden="true"
      >
        +
      </span>
...
function ProjectUploadDialog({
  ...
  const projectTargetControl = isProjectSelectionFixed ? (
    <div className="flex min-w-0 items-center gap-3 rounded-lg border border-violet-400/25 bg-violet-500/[0.08] px-3.5 py-3 text-sm sm:max-w-md">
      <FolderOpen
        className="size-4 shrink-0 text-violet-200"
        aria-hidden="true"
      />
      <span className="shrink-0 font-semibold text-violet-100">
        {t("uploadToProject")}
      </span>
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      ariaLabel={t("uploadAssets")}
      closeLabel={t("close")}
      contentClassName="max-w-[46rem] sm:overflow-hidden"
      keepMounted
    >
...
function ProjectSkeleton() {
  return (
    <div
      className="workspace-glass-surface min-h-[13.875rem] animate-pulse rounded-xl border p-5"
      aria-hidden="true"
    >
...
export function ProjectsDashboard({
  ...
  const [selectedUploadProjectId, setSelectedUploadProjectId] = useState<
    Project["_id"] | null
  >(null);
  const [uploadEntrySource, setUploadEntrySource] =
  ...
  const completionTimersRef = useRef<
    Map<string, { dismiss: number; remove: number }>
  >(new Map());
  const router = useRouter();
  ...
  async function swapProjects(
    ...
    [reorderedProjects[sourceIndex], reorderedProjects[targetIndex]] = [
  ...
  return (
    <section
      className="-mx-4 -my-7 flex h-[calc(100dvh-3.5rem)] flex-col px-4 py-7 sm:-mx-6 sm:px-6 lg:-mx-8 lg:-my-8 lg:px-8 lg:py-8 xl:-mx-10 xl:px-10"
    ...
    >
      <div className="shrink-0 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            type="button"
            className="border border-blue-500/80 bg-blue-700 text-white shadow-[0_6px_18px_rgba(37,99,235,0.25)] hover:bg-blue-800 hover:text-white"
          ...
          <Button
            type="button"
            className="bg-green-700 text-white shadow-[0_6px_18px_rgba(21,128,61,0.22)] hover:bg-green-800 hover:text-white"
            onClick={() => openUpload()}
      ...
      <div className="mt-8 min-h-0 flex-1">
        {projects === undefined ? (
          <div className="flex h-full min-h-0 flex-col" aria-busy="true">
            ...
            <div
              className="h-4 w-16 animate-pulse rounded bg-white/[0.07]"
              aria-hidden="true"
            />
