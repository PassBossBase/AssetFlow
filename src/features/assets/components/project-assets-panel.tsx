...
import { Check, ChevronDown, FolderInput, ImageOff, Search, SearchX, SlidersHorizontal, Trash2, X } from "lucide-react";
...
function AssetCard({ asset, isSelected, onSelectedChange }: { asset: Asset; isSelected: boolean; onSelectedChange: (selected: boolean) => void }) {
  ...
  return (
    <Card className={`workspace-glass-surface group transition-colors ${isSelected ? "border-primary/60 bg-primary/[0.045] shadow-[0_0_0_1px_rgb(56_217_245_/_0.13)]" : ""}`}>
      <CardContent className="relative p-3 pb-0">
        <label className="absolute left-5 top-5 z-10 flex size-6 cursor-pointer items-center justify-center rounded-md border border-white/[0.22] bg-[#08131f]/90 shadow-[0_2px_8px_rgb(0_0_0_/_0.24)] backdrop-blur-sm transition-colors hover:border-primary/80">
          <span className="sr-only">{t("selectAsset").replace("{name}", asset.name)}</span>
          <input
            type="checkbox"
...
function BatchMoveControl({ disabled, onMove, projects, selectedCount, sourceProjectId }: {
  disabled: boolean;
  ...
  const targets = (projects ?? []).filter((project) => project._id !== sourceProjectId);
  ...
  return (
    <Popover
      ...
      popupWidth={320}
      trigger={(
        <Button type="button" variant="ghost" size="sm" className="h-10 border border-violet-400/65 bg-violet-700 px-3 text-white shadow-[0_6px_16px_rgb(109_40_217_/_0.18)] hover:bg-violet-800 hover:text-white" disabled={disabled}>
      ...
      content={(
        <div>
          <div className="border-b border-white/[0.08] px-4 py-3.5">
            <p className="text-sm font-semibold text-foreground">{t("moveSelectedAssets")}</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">{t("moveSelectedAssetsDescription").replace("{count}", String(selectedCount))}</p>
          </div>
          {targets.length === 0 ? <p className="px-4 py-5 text-sm text-muted-foreground">{t("noOtherProjects")}</p> : (
...
export function ProjectAssetsPanel({ projectId }: { projectId: Id<"projects"> }) {
  ...
  const areAllAssetsSelected = assets !== undefined && assets.length > 0 && assets.every((asset) => selectedAssetIds.has(asset._id));

  function handleAssetSelectedChange(assetId: Id<"assets">, selected: boolean) {
    if (selected && !selectedAssetIds.has(assetId) && selectedAssetIds.size >= maxBatchAssetCount) {
      toast.add({ type: "error", title: t("batchSelectionLimit").replace("{count}", String(maxBatchAssetCount)) });
      return;
    }
  ...
  function handleToggleAllAssets() {
    ...
    setSelectedAssetIds(new Set(assets.slice(0, maxBatchAssetCount).map((asset) => asset._id)));
  ...
  async function handleBatchMove(targetProjectId: Id<"projects">) {
    ...
    try {
      const movedCount = await batchMoveAssets({ ids, projectId, targetProjectId });
      setSelectedAssetIds(new Set());
      toast.add({ type: "success", title: t("assetsMoved").replace("{count}", String(movedCount)) });
      return true;
    } catch (moveError) {
      toast.add({ type: "error", title: moveError instanceof Error ? moveError.message : t("couldNotMoveAssets"), priority: "high" });
      return false;
    } finally {
  ...
  async function handleBatchRemove() {
    ...
    try {
      ...
      setSelectedAssetIds(new Set());
      toast.add({ type: "success", title: t("assetsDeleted").replace("{count}", String(removedCount)) });
    } catch (removeError) {
      toast.add({ type: "error", title: removeError instanceof Error ? removeError.message : t("couldNotDeleteAssets"), priority: "high" });
    } finally {
        ...
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="group flex h-11 min-w-0 items-center gap-2 rounded-xl border border-white/[0.12] bg-[#0a1422]/85 px-3 shadow-[inset_0_1px_0_rgb(255_255_255_/_0.035)] transition-colors focus-within:border-primary/55 focus-within:bg-[#0d1c2e] focus-within:ring-2 focus-within:ring-primary/20 sm:w-60">
              ...
              <input className="min-w-0 flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground" value={search} onChange={(event) => handleSearchChange(event.target.value)} placeholder={t("searchAssets")} />
            </label>
            <div className="w-full sm:w-44"><AssetTypeFilter extension={extension} onChange={handleExtensionChange} /></div>
            <Button type="button" variant="ghost" size="sm" className="h-10 border border-blue-400/65 bg-blue-700 px-3 text-white shadow-[0_6px_16px_rgb(37_99_235_/_0.18)] hover:bg-blue-800 hover:text-white" onClick={handleToggleAllAssets} disabled={assets === undefined || assets.length === 0 || isBatchDeleting || isBatchMoving}>
              <Check className="size-4 text-white" aria-hidden="true" />
              {areAllAssetsSelected ? t("clearSelection") : t("selectAllAssets")}
            </Button>
            <div className="flex flex-wrap items-center gap-2 sm:border-l sm:border-white/[0.1] sm:pl-2" role="group" aria-label={t("batchActions")}>
              ...
              <BatchMoveControl disabled={selectedCount === 0 || isBatchDeleting || isBatchMoving} onMove={handleBatchMove} projects={projects} selectedCount={selectedCount} sourceProjectId={projectId} />
              <PopConfirm
                title={t("deleteSelectedAssetsTitle")}
                description={t("deleteSelectedAssetsConfirm").replace("{count}", String(selectedCount))}
                confirmLabel={isBatchDeleting ? t("deleting") : t("deleteSelectedAssets")}
                cancelLabel={t("cancel")}
                disabled={selectedCount === 0 || isBatchDeleting || isBatchMoving}
                onConfirm={handleBatchRemove}
                trigger={<Button type="button" variant="destructive" size="sm" className="h-10 border border-[#fa807b]/65 bg-[#c83e3b] px-3 text-white shadow-[0_6px_16px_rgb(200_62_59_/_0.16)] hover:bg-[#de4b47] hover:text-white" disabled={selectedCount === 0 || isBatchDeleting || isBatchMoving} aria-busy={isBatchDeleting}><Trash2 className="size-4" aria-hidden="true" />{t("deleteSelectedAssets")}</Button>}
              />
              <Button type="button" variant="ghost" size="sm" className="h-10 border border-slate-300/50 bg-slate-700 px-2.5 text-white shadow-[0_6px_16px_rgb(51_65_85_/_0.15)] hover:bg-slate-600 hover:text-white" onClick={() => setSelectedAssetIds(new Set())} disabled={selectedCount === 0 || isBatchDeleting || isBatchMoving}>
